...
print("Label mapping:", label_mapping)
#Label mapping: {'moderate': 0, 'strong': 1, 'weak': 2}
...
# Get the trained XGBoost model and transformed test data
...
# === SHAP Waterfall Plots per Sample and Class ===
# === SHAP Waterfall Plots per Sample and Class + CSV dump ===
output_folder = str(OUTPUT_DIR / str(OUTPUT_DIR / "shap_waterfall_plots_all_classes"))
os.makedirs(output_folder, exist_ok=True)

print("🧬 Saving SHAP waterfall plots + feature tables for each test instance and each class...")

# Get feature names from fitted preprocessor (ensures alignment)
...
for i in range(len(X_test)):
    ...
    for class_idx in range(num_classes):
        ...
        # Build DataFrame with feature values + shap values
        df_row = pd.DataFrame({
            "feature": feature_names,
        ...
        # Plot waterfall (no extra info panel)
